...
import cyberclaw.channels.matrix as matrix_module
from cyberclaw.bus.events import OutboundMessage
from cyberclaw.bus.queue import MessageBus
from cyberclaw.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from cyberclaw.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("cyberclaw.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "cyberclaw.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("cyberclaw.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "cyberclaw.channels.matrix.asyncio.create_task", _fake_create_task
    )
