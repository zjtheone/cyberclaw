...
import cyberclaw.channels.dingtalk as dingtalk_module
from cyberclaw.channels.dingtalk import DingTalkChannel, CyberclawDingTalkHandler
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "Cyberclaw Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = CyberclawDingTalkHandler(channel)
