...
from cyberclaw import __logo__, __version__
from cyberclaw.config.paths import get_workspace_path
from cyberclaw.config.schema import Config
from cyberclaw.utils.helpers import sync_workspace_templates

app = typer.Typer(
    name="cyberclaw",
    help=f"{__logo__} cyberclaw - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from cyberclaw.config.paths import get_cli_history_path
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} cyberclaw[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} cyberclaw v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """cyberclaw - Personal AI Assistant."""
    pass
...
@app.command()
def onboard():
    """Initialize cyberclaw configuration and workspace."""
    from cyberclaw.config.loader import get_config_path, load_config, save_config
    from cyberclaw.config.schema import Config
    ...
    console.print(f"\n{__logo__} cyberclaw is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.cyberclaw/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]cyberclaw agent -m \"Hello!\"[/cyan]")
    console.print("\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/cyberclaw#-chat-apps[/dim]")
...
def _make_provider(config: Config):
    ...
    from cyberclaw.providers.custom_provider import CustomProvider
    ...
    if provider_name == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            ...
            console.print("Set them in ~/.cyberclaw/config.json under providers.azure_openai section")
    ...
    from cyberclaw.providers.registry import find_by_name
    spec = find_by_name(provider_name)
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and spec.is_oauth):
        console.print("[red]Error: No API key configured.[/red]")
        console.print("Set one in ~/.cyberclaw/config.json under providers section")
        raise typer.Exit(1)
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from cyberclaw.config.loader import load_config, set_config_path
...
def _print_deprecated_memory_window_notice(config: Config) -> None:
    ...
    if config.agents.defaults.should_warn_deprecated_memory_window:
        console.print(
            ...
            "[cyan]cyberclaw onboard[/cyan] to refresh your config template."
        )
...
def gateway(
    ...
    from cyberclaw.channels.manager import ChannelManager
    from cyberclaw.config.paths import get_cron_dir
    from cyberclaw.cron.service import CronService
    from cyberclaw.cron.types import CronJob
    from cyberclaw.heartbeat.service import HeartbeatService
    from cyberclaw.session.manager import SessionManager
    ...
    console.print(f"{__logo__} Starting cyberclaw gateway on port {port}...")
    ...
    if multi:
        ...
        console.print(
            f"{__logo__} Starting cyberclaw gateway in MULTI-AGENT mode on port {port}..."
        )
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show cyberclaw runtime logs during chat"),
    ...
    if logs:
        logger.enable("cyberclaw")
    else:
        logger.disable("cyberclaw")
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]cyberclaw is thinking...[/dim]", spinner="dots")
    ...
    if message:
        ...
        from cyberclaw.bus.events import InboundMessage
...
def cron_add(
    ...
    from cyberclaw.config.paths import get_cron_dir
    from cyberclaw.cron.service import CronService
    from cyberclaw.cron.types import CronSchedule
...
def _get_bridge_dir() -> Path:
    ...
    from cyberclaw.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # cyberclaw/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall cyberclaw")
        raise typer.Exit(1)
...
def channels_login():
    ...
    from cyberclaw.config.loader import load_config
    from cyberclaw.config.paths import get_runtime_subdir
...
def status():
    ...
    console.print(f"{__logo__} cyberclaw Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from cyberclaw.providers.registry import PROVIDERS
...
try:
    from cyberclaw.cli.subagents import subagents_app
    app.add_typer(subagents_app, name="subagents", help="Manage subagent runs")
except ImportError:
    pass

try:
    from cyberclaw.cli.sessions import sessions_app
